...
class MyCustomLibrary():
    ...
    def editJsonFromDict(self, inputJsonFile, dicFile):
        """
        ...
        # 如果要输出一个替换后的文件，加入一个outputfile传参，然后再写到该文件即可
        fin = open(inputJsonFile, 'r')  # 打开原始文件
        fdic = open(dicFile, 'rU')
        keyValue = fdic.readline()
    ...
    def getUserTrace(self, os):
        ...
        #              {"event": "Activity2#OnResume", "time": utcTime - 18000}]
            ...
            trace = [{"event": "Session Start", "time": utcTime - 25000},
                     {"event": "DemoActivity#OnCreate", "time": utcTime - 22000},
                     {"event": "DemoActivity#OnResume", "time": utcTime - 20000}]
        return trace
