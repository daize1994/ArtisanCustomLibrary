...
import httplib, urllib2,urllib,requests
import time
import json
import sys
...
class MyCustomLibrary():
    ...
    def editJson(self, inputJsonFile,**keyValue):
    	"""功能：替换指定json文件中的多个指定key的值，并覆盖该文件
    	`inputJsonFile` 待替换文件
    	`keyValue` 接受多对待替换的k-v,v为新值
    ...
    def getUserTrace(self,os,activity_index=0):
        ...
        #print utcTime
        if os == 'ios':
            trace = [{"event":"Session Start","time":utcTime-25000},{"event":"UINavigationController#viewWillAppear","time":utcTime-22000},{"event":"ViewController#viewWillAppear","time":utcTime-20000}]
        elif os == 'android' and 0<=activity_index<=2:
            trace =[{"event":"Session Start","time":utcTime-25000},{"event":"MainActivity#OnCreate","time":utcTime-22000},{"event":"MainActivity#OnResume","time":utcTime-20000}]
        elif os == 'android' and 3<=activity_index <= 4:
            trace =[{"event":"Session Start","time":utcTime-25000},{"event":"MainActivity#OnCreate","time":utcTime-122000},{"event":"MainActivity#OnResume","time":utcTime-20000},{"event":"Activity1#OnCreate","time":utcTime-19000},{"event":"Activity1#OnResume","time":utcTime-18000}]
        elif os == 'android' and 5<=activity_index <= 6:
            trace = [{"event":"Session Start","time":utcTime-25000},{"event":"MainActivity#OnCreate","time":utcTime-22000},{"event":"MainActivity#OnResume","time":utcTime-20000},{"event":"Activity2#OnCreate","time":utcTime-19000},{"event":"Activity2#OnResume","time":utcTime-18000}]
        else:
            trace =[{"event":"Session Start","time":utcTime-25000},{"event":"MainActivity#OnCreate","time":utcTime-22000},{"event":"MainActivity#OnResume","time":utcTime-20000}]
        #print trace
        return trace